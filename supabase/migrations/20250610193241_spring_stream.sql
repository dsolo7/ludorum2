/*
  ...
  1. New Tables
    - `contests` - Contest definitions with token costs and prize pools
    - `contest_entries` - User entries in contests with predictions
    - `prediction_cards` - Prediction questions linked to contests
    - `social_votes` - Community voting on analyzer results

  2. Security
    - Enable RLS on all tables
    - Policies for user access control and admin management
    - Proper foreign key constraints

  3. Functions & Triggers
    - Auto-update contest entry counts
    - Social voting statistics
...
-- Drop existing function completely before recreating with new signature
DROP FUNCTION IF EXISTS get_social_stats(uuid);

-- Create function to get social stats with proper return type
