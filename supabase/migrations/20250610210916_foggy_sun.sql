/*
  ...
  1. New Tables
    - `page_layouts`: Stores page metadata and configuration
    - `ui_blocks`: Stores individual UI blocks that make up pages

  2. Security
    - Enable RLS on all tables
    - Add policies for admin access
    - Add policies for public viewing of published pages
...
-- Create policies for ui_blocks
CREATE POLICY "Admins can manage UI blocks"
  ON ui_blocks
  FOR ALL
  TO authenticated
  USING (
...
CREATE INDEX idx_ui_blocks_page_id ON ui_blocks(page_id);
CREATE INDEX idx_ui_blocks_block_type ON ui_blocks(block_type);
CREATE INDEX idx_ui_blocks_position ON ui_blocks(position);
...
CREATE OR REPLACE FUNCTION update_ui_blocks_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
...
CREATE TRIGGER update_ui_blocks_updated_at
